...
$$  
V_{\pi}(s)=\sum_{a\in A}\pi(a\mid s)(R(s,a)+\gamma\sum_{s^{\prime}\in S}p(s^{\prime}\mid s,a)V_{\pi}(s^{\prime})  
$$  
对于b:  
$$  
V_{\pi}(s)=\sum_{a\in A}\pi(a\mid s)Q_{\pi}(s,a)  
...
带入得到a式  
## 策略评估  
计算 $\colon V_{\pi}(s)$ 的过程就是策略评估  
用贝尔曼方程迭代得到价值函数  
$$  
V_{\pi}^{k}(s)=r(s,\pi(s))+\gamma\sum_{s^{\prime}\in{\cal S}}p\left(s^{\prime}\mid s,\pi(s)\right)V_{\pi}^{k-1}\left(s^{\prime}\right)  
$$  
- $k$ 是迭代次数  
## 预测和控制  
预测:给定策略,输出价值函数  
- 输入:马尔可夫决策过程 $<S,A,P,R,\gamma>$ 和策略  $\pi$  
- 输出:价值函数 $V_{\pi}$   
控制:寻找最佳策略,输出最佳价值函数和最佳策略  
- 输入:马尔可夫决策过程 $<S,A,P,R,\gamma>$   
- 输出:最佳价值函数 ${\mathcal{V}}^*$ 和最佳策略 $\pi^*$   
都是用动态规划解决  
通过解决预测问题,从而解决控制问题  
## 动态规划  
dynamic programming，DP  
解决满足 最优子结构optimal substructure 和 重叠子问题overlapping subproblem  
- 最优子结构:将问题拆分成小问题,组合小问题的答案得到最优解  
- 重叠子问题:子问题出现多次,子问题的解决方案可以重复使用  
## 马尔可夫决策过程中的策略评估  
给定马尔可夫决策过程和策略,即对于当前策略,我们可以得到多大的价值  
把**贝尔曼期望备份（Bellman expectation backup）**,变成迭代的过程  
可以看作 **同步备份synchronous backup**  
- 每一次的迭代都会完全更新所有的状态  
- 异步备份（asynchronous backup）的思想就是通过某种方式，使得每一次迭代不需要更新所有的状态  
将贝尔曼期望备份转换成动态规划的迭代  
$$  
...
已给定策略,将 $a$ 从式中去除  
$$  
V^{t+1}(s)=r_\pi(s)+\gamma P_\pi\left(s^{\prime}\mid s\right)V^{t}\left(s^{\prime}\right)  
$$  
迭代上式,得到每个状态的价值,在马尔可夫决策过程中,价值函数 $V$ 的变量只与状态有关  
## 马尔可夫决策过程中的控制  
只有马尔可夫决策过程  
寻找 最佳的策略 从而寻找 最佳价值函数  
最佳价值函数  
$$  
V^{*}(s)={max_\pi}V_{\pi}(s)  
$$  
最佳策略  
$$  
\pi^{*}(s){=}\arg\operatorname*{max}_{\pi}V_{\pi}(s)  
$$  
如果可以得到最佳价值函数,就认为该马尔可夫决策过程可解  
最佳价值函数是一致的,所以环境中可达到的上限的值是一致的,但可能有多个价值策略  
当取得最佳价值函数后,通过对Q函数进行最大化来取得最佳价值策略  
$$  
\pi^{*}(a\mid s)=\left\{\begin{array}{l l}{{1,}}&{\arg\operatorname*{max}_{a\in A}Q^*(s,a)}\\ {{0,}}&{{other}}\end{array}\right.  
$$  
## 策略迭代  
- 策略评估  
- 策略改进  

![[Pasted image 20250925093000.png]]  
1. 评估  
	- 根据现有策略得出该策略下的状态价值函数  
2. 改进  
	- 得到状态价值函数后,推算出Q函数,对Q函数最大化处理,得到新的策略  
### 策略评估  
通过策略函数 计算 状态价值函数  
### 策略改进  
得到状态价值函数后,通过奖励函数 与 状态转移函数 计算Q函数  
$$  
{Q}_{\pi_{i}}(s,a)=R(s,a)+\gamma\sum_{s^{\prime}\in{\cal S}}p\left(s^{\prime}\mid s,a\right)V_{\pi_{i}}\left(s^{\prime}\right)  
$$  
贪心化Q函数,计算新一轮策略函数  
$$  
\pi_{i+1}(s)=\arg\operatorname*{max}_{a}Q_{\pi_{i}}(s,a)  
$$  
可将Q函数视为一个Q表格  
![[Pasted image 20250925101511.png]]  
得到Q函数,就得到了Q表格  
对于每个状态,取最大值的动作作为策略  
### 贝尔曼最优方程  
Q函数不断取贪心直至收敛,  
停止时,取让Q函数值最大化的动作,Q函数就变成了状态价值函数  
$$  
Q_{\pi}\left(s,\pi^{\prime}(s)\right)=\operatorname*{max}_{a\in A}Q_{\pi}(s,a)=Q_{\pi}(s,\pi(s))=V_{\pi}(s)  
$$  
即贝尔曼最优方程  
$$  
V_{\pi}(s)\mathop{=}\operatorname*{max}_{a\in A}{Q}_{\pi}(s,a)  
$$  
- 最佳策略下的下一个状态的价值 必须等于 在这个状态采取最好动作得到的回报的期望  
只有当得到最佳价值函数后,贝尔曼方程才会满足  
取最大化操作  
$$  
V^*(s)\mathop{=}\operatorname*{max}_{a}{Q^*}(s,a)  
$$  
代入Q函数的贝尔曼方程  
$$  
...
$$  
$$  
Q^{*}(s,a)=R(s,a)+\gamma\sum_{s^{\prime}\in S}p\left(s^{\prime}\mid s,a\right)\operatorname*{max}_{a^{\prime}}Q^{*}\left(s^{\prime},a^{\prime}\right)  
$$  
## 价值迭代  
### 最优性原理定理  
一个策略 $\pi(a|s)$ 在状态 $s$ 时达到最优值,即 $V_{\pi}(s)\ =\ V^{*}(s)$ 当且仅当 任何从 $s$ 到达的 $s'$ 都已经达到最优价值,即对于所有 $s'$ , $V_{\pi}(s)\ =\ V^{*}(s)$ 恒成立  
### 确定性价值迭代  
当知道子问题 $V_{\pi}(s^{\prime})$ 的最优解,就可以通过价值迭代得到最优的 $V^{*}(s^{\prime})$  
价值迭代的规则,根据贝尔曼最优方程  
$$  
V(s){\leftarrow\mathrm{max}_{a\in A}}\left(R(s,a)+\gamma\sum_{s^{\prime}\in S}p\left(s^{\prime}\mid s,a\right)V\left(s^{\prime}\right)\right)  
$$  
### 价值迭代算法  
